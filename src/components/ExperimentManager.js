...
const Card = ({ title, value, color, expName, date }) => {
  ...
  if (date) {
    ...
    if (!isNaN(d.getTime())) {
      ...
      displayDate = `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(
        d.getDate()
...
const ExperimentManager = () => {
  ...
  async function loadExperiments() {
    ...
    try {
      ...
      setExperiments(
        normalized.filter(
          (exp) => ensureArray(exp.improvements).length > 0 && exp.isValid
        )
      );
        ...
        .flatMap((exp) =>
          ensureArray(exp.goals)
            .filter((goal) => {
              ...
              const improvements = ensureArray(exp.improvements).map((i) =>
      ...
      types.forEach((type) => {
        ...
        if (arr.length > 0) {
          const maxGoal = arr.reduce(
            (max, curr) =>
              max == null || (curr.value != null && curr.value > max.value)
                ? curr
                : max,
            null
          );
  ...
  return (
    ...
    >
      ...
      >
          ...
          >
            ...
            {experiments
              ...
              .map((exp) => {
                ...
                const colors = [];
                if (improvements.includes("Open"))
                  colors.push(chartColors.open.border);
                if (improvements.includes("Close"))
                  colors.push(chartColors.close.border);
                if (improvements.includes("Reg"))
                  colors.push(chartColors.reg.border);
        ...
        >
          <div
            style={{ marginLeft: "auto", color: theme.colors.text.secondary }}
          >
            Select plot type
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
          ...
          <button
            onClick={() => setPlotType("classification")}
            style={{
              padding: "8px 12px",
              borderRadius: "6px",
              border: `1px solid ${theme.colors.border}`,
